...
- Pass the user's task text as a single quoted argument. Never let it be
  expanded by the shell: if it contains `$`, a backtick or a quote, use a
  heredoc with a quoted delimiter (`<<'EOF'`) instead of inlining it.
