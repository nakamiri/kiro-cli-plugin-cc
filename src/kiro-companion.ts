 ...
 */
export function reviewArgs(rawArgs: string[]): { base: string; extra: string } {
...
function startRunner(kind: string, kiro: string, prompt: string, timeoutMs: number): Job | string {
  ...
  try {
    child = spawn(
      nodeBinary(),
      [runnerPath(), job.id, String(timeoutMs), kiro, ...chatArgs(prompt, kind)],
      {
        stdio: "ignore",
        detached: true,
...
export function reviewContext(base: string): string {
 ...
 */
function bodyOf(job: Job): string {
  let stored: string | null;
  try {
    stored = readJobResult(job.id);
  } catch (e) {
    const size = job.resultBytes !== undefined ? `${job.resultBytes} bytes of` : "stored";
    return `ERROR: job ${job.id} has ${size} output, but it could not be read: ${(e as Error).message}`;
  }
...
export function cancel(args: string[]): string {
  ...
  try {
    ...
    return (
      `Job ${job.id} was signalled, but its record could not be read to update it: ` +
      `${(e as Error).message}`
    );
  }
  if (base === null) {
