...
function collected(): string {
  if (stripAnsiEnabled) {
    ...
    for (const held of [outStripper.end(), errStripper.end()]) {
      if (held !== "") append(held, Buffer.byteLength(held));
    }
  }
...
function leadsOwnGroup(): boolean {
  ...
  if (process.env.KIRO_PLUGIN_RUNNER_DETACHED === "1") return true;
  try {
...
function take(stripper: AnsiStripper, chunk: string): void {
  const text = stripAnsiEnabled ? stripper.write(chunk) : chunk;
  if (text !== "") append(text, Buffer.byteLength(text));
}

child.stdout?.on("data", (d: string) => take(outStripper, d));
child.stderr?.on("data", (d: string) => take(errStripper, d));
