...
export type Kind = "review" | "rescue";
 ...
 * run left it.
 */
export function agentName(kind: Kind, jobId: string): string {
...
type Rule = { capability: string; match: string[]; effect: "allow" | "deny" };
...
const TOOLS: Record<Kind, string[]> = {
...
export function agentConfig(kind: Kind, jobId: string): object {
  return {
...
export function agentPath(kind: Kind, jobId: string, cwd: string): string {
  return join(agentsDir(cwd), `${agentName(kind, jobId)}.json`);
}

/** What was created, so teardown removes exactly that and no more. */
export type InstalledAgent = { name: string; path: string; createdDirs: string[] };
 ...
 */
export function installAgent(kind: Kind, jobId: string, cwd: string): InstalledAgent {
  ...
  for (const d of [join(cwd, ".kiro"), dir]) {
    if (!existsSync(d)) createdDirs.push(d);
  }
 ...
 */
export function removeAgent(installed: InstalledAgent): void {
  try {
 ...
 */
export function sweepStaleAgents(cwd: string, maxAgeMs: number, statMtimeMs: (p: string) => number): number {
  const dir = agentsDir(cwd);
  let names: string[];
  try {
  ...
  for (const name of names) {
    if (!GENERATED_AGENT_RE.test(name)) continue;
    const path = join(dir, name);
    try {
      if (Date.now() - statMtimeMs(path) < maxAgeMs) continue;
      unlinkSync(path);
      removed += 1;
    } catch {
...
export function describeRules(kind: Kind): string[] {
