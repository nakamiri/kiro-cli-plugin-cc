...
function readJobFileOrSkip(path: string, expectedId: string): Job | null {
  try {
...
export function finishedTime(job: Job): number {
 ...
 */
function transcriptBytes(dir: string, job: Job): number {
  if (job.resultBytes !== undefined) return job.resultBytes;
  try {
 ...
 */
function migrateLegacyJobsDir(target: string): void {
...
export function pruneJobs(): void {
  ...
  for (const name of names) {
    ...
    if (tmp) {
      ...
      if (isPidAlive(Number(tmp[1])) && ageOf(dir, name) < TMP_INFLIGHT_MS) continue;
  ...
  for (const job of terminal) {
    if (finishedTime(job) < cutoff) doomed.add(job.id);
  }
